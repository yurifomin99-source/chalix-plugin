$(document).ready(function() {
    ...
    const themeCookie = 'chalix_theme-toggle-dark';

    function applyThemeOnPage(){
      const theme = $.cookie(themeCookie);
      {% if CHALIX_THEME_ENABLE_DARK_TOGGLE %}
      $('body').toggleClass("chalix_theme-dark-theme", theme === 'dark');       // append or remove dark-class based on cookie-value
      // update expiry
    ...
    window.addEventListener("message", function(e){
      if (e.data && e.data["chalix_theme-toggle-dark"]){
        applyThemeOnPage();
      }
    });
